...
#ifdef HAVE_MRA_TTG
vecfuncT SCF::apply_bsh_mrattg(World& world, vecfuncT& Vpsi, const tensorT& eps,
                               const CalculationParameters& param) {
    ...
    ops.clear();
    // TODO: move Vpsi into load-vmra so we can free nodes once they are loaded
    Vpsi.clear();
...
vecfuncT SCF::compute_residual(World& world, tensorT& occ, tensorT& fock,
    ...
    // across chunks (restored after the loop).
    {
        ...
        for (size_t ilo = 0; ilo < size_t(nmo); ilo += ntile) {
            ...
            for (size_t i = ilo; i < iend; ++i)
                Vpsi[i].gaxpy(-2.0, fpsi[i - ilo], 2.0, false);
            world.gop.fence();
